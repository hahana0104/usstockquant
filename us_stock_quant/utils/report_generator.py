...
def generate_html_report(backtest_data: dict) -> str:
    ...
    # 生成图表数据（使用Plotly.js嵌入，不需要Kaleido）
    chart_div = ""
    if not equity_curve.empty:
        # 准备数据
        dates = equity_curve['date'].tolist()
        equities = equity_curve['equity'].tolist()
        
        # 计算归一化
        initial = equities[0] if equities else 1
        normalized = [e / initial for e in equities]
