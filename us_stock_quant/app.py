    ...
    if run_button:
        ...
        try:
            ...
            if selected_strategy == 'three_factor':
                ...
                # 技术指标策略 - 为多只股票分别计算信号，选择最强的
                from strategies.all_strategies import get_strategy
                
                strategy_obj = get_strategy(selected_strategy, strategy_params)
                rebalance_dates = get_rebalance_dates(
                    start_date.strftime('%Y-%m-%d'),
                    end_date.strftime('%Y-%m-%d')
                )
                ...
                for date in rebalance_dates:
                    try:
                        ...
                        for ticker in tickers:
                            try:
                                if ticker in price_data['Close'].columns:
                                    # 获取该股票的价格数据
                                    ticker_prices = price_data.xs(ticker, level='Ticker', axis=1) if 'Ticker' in price_data.columns.names else price_data['Close'][ticker].to_frame()
                                    if isinstance(ticker_prices, pd.Series):
                                        ticker_prices = ticker_prices.to_frame('Close')
                                    
                                    # 计算到当前日期的数据
                                    mask = ticker_prices.index <= date
                                    hist_data = ticker_prices[mask]
                                    
                                    if len(hist_data) > 50:  # 确保有足够数据
                                        # 生成信号
                                        signal_df = strategy_obj.generate_signals(hist_data)
                                        if 'signal' in signal_df.columns:
                                            # 使用最新的信号值作为得分
                                            latest_signal = signal_df['signal'].iloc[-1]
                                            ticker_scores[ticker] = latest_signal
                            except:
                                continue
                        ...
                        if ticker_scores:
                            ...
                            # 选择前20%的股票
                            n_select = max(1, len(sorted_tickers) // 5)
                            selected = [t for t, s in sorted_tickers[:n_select] if s > 0]
                ...
                if not all_signals:
                    st.error("无法生成交易信号，请检查股票代码或调整参数")
                    return
