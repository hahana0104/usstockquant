...
if df.empty:
    print(f"数据下载失败! 股票代码 {ticker} 可能不存在或没有数据。")
    print("\n尝试搜索相关股票...")
    
    # 尝试类似代码
    alternatives = ['CIRC', 'CRCL', 'CRL', 'CCL', 'CRWD']
    for alt in alternatives:
        try:
            test = yf.Ticker(alt).history(period='5d')
            if not test.empty:
                print(f"  找到: {alt} - 有数据")
        except:
            pass
...
# 保存结果
df.to_csv(r'C:\Users\nono\.openclaw\workspace\us_stock_quant\CRCL_backtest.csv')
print(f"\n详细数据已保存: CRCL_backtest.csv")
