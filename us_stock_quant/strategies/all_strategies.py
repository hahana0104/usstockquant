...
class ThreeFactorStrategy(BaseStrategy):
    ...
    def get_params_config(self) -> Dict:
        return {
            'quality_weight': {'type': 'float', 'min': 0, 'max': 1, 'default': 0.4, 'label': '质量因子权重'},
            'value_weight': {'type': 'float', 'min': 0, 'max': 1, 'default': 0.3, 'label': '价值因子权重'},
            'momentum_weight': {'type': 'float', 'min': 0, 'max': 1, 'default': 0.3, 'label': '动量因子权重'},
